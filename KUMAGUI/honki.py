...
class MyFrame2(wx.Frame):
    ...
    def calc_and_display(self, event):  # wxGlade: MyFrame2.<event_handler>
        # common simulation for helical/single schemes
        dist = float(self.camelenBox.GetValue())
        wavelength = float(self.wavelenBox.GetValue())
        twotheta = math.atan((detectordim / 2.0) / dist)
        theta = twotheta / 2.0
        mind = wavelength / 2.0 / math.sin(theta)
        mindstr = "%8.2f" % mind
        self.edgeResolSimuBox.SetValue(mindstr)

        # Beamsize index get
        bs_index_gui = self.beamsizePullDown.GetCurrentSelection()
        bs_index = bs_index_gui + 1
        print "Beamsize index=", bs_index
        flux = self.getFlux(wavelength, mode="helical_and_single")
        flux_str = "%5.3e" % flux
        self.FluxBox.SetValue(flux_str)
        ...
        isHelical = self.checkbox_1.GetValue()

        # making a 'cond' dictionary for KUMA dose estimation
        kumacond = dict(dose_ds=aimed_dose, wavelength=wavelength, exp_ds=exptime, total_osc=phi_total, osc_width=osc_step,
            ds_vbeam=beamv_um, ds_hbeam=beamh_um, reduced_fact=1.0, ntimes=1)

        kuma = KUMA.KUMA()

        # Dose estimation by using KUMA
        if isHelical:
            # startvec
            ...
            # dist_vec_mm along Y direction
            dist_vec_mm = numpy.abs(scanvec[1])
            # Crystal size along Y direction
            crysize_um = dist_vec_mm * 1000.0
            diststr = "%8.4f" % crysize_um
            self.advancedLength.SetValue(diststr)
            mod_exp, mod_trans = kuma.getBestCondsHelical(kumacond, flux, dist_vec_mm)
            # calculating total exposure time for this data collection
            total_exp = mod_exp * float(nframes)
            transpeed = crysize_um / total_exp
            trspd_str = "%6.2f" % transpeed
            self.transSpeedBox.SetValue(trspd_str)

        # for single irradiation point experiment
        else:
            mod_exp, mod_trans = kuma.getBestCondsMulti(kumacond, flux)
            self.transSpeedBox.SetValue("-")
            self.advancedLength.SetValue("-")

        # Displaying calculated values on GUI
        total_exp = mod_exp * float(nframes)
        te_str = "%5.1f" % total_exp
        self.totalExpBox.SetValue(te_str)
        self.exptimeBox.SetValue("%5.2f" % mod_exp)

        self.logfile.write("Selected beam size = %8.2f %8.2f\n" % (beamh_um, beamv_um))

        # Attenuator selection
        real_thick = self.attfac.calcThickness(wavelength, mod_trans)

        if beamline == "BL45XU" or beamline == "BL32XU":
            best_thick = self.attfac.getBestAtt(wavelength, mod_trans)
            thickstr = "%4.0f" % best_thick
            self.dispSuitAttBox.SetValue(thickstr)
        else:
            best_thick = self.attfac.calcThickness(wavelength, mod_trans)
            thickstr = "%4.0f" % best_thick
            self.dispSuitAttBox.SetValue(thickstr)

        transstr = "%6.4f" % (mod_trans * 100.0) + " %"
        self.dispARDbox.SetValue(transstr)
        self.attSimuBox.SetValue("%6.4f" % (mod_trans * 100.0))
        ...
        # Optimism count rate
        count_rate = flux * mod_trans / det_area
        ocr_str = "%5.1e" % count_rate
    ...
    def pushOutput(self, event):  # wxGlade: MyFrame2.<event_handler>
        ...
        self.calc_and_display(event)

        # Preparation of making schedule file
        # Constructor
        ...
        # Setting transmission to schedule file class
        if beamline == "BL45XU":
            # attenuator
            ...
            # Transmission should be set in [%]
            trans_percent = trans * 100.0
            print "Current transmission is %8.2f percent" % trans_percent
            bsssch.setTrans(trans_percent)

        if isHelical:
            # crystal length [um->mm]
            crylen = float(self.advancedLength.GetValue()) / 1000.0
            phi_total = endphi - startphi
            nframes = int(phi_total / stepphi)
    ...
    def genMulti(self, event):  # wxGlade: MyFrame2.<event_handler>
        ...
        if beamline == "BL45XU":
            ...
            mlc.makeGUI(outdir, wl, gonio_list, dist, startphi, endphi, oscwid, best_thick, beamsize_index=bs_index)
        else:
            mlc.setExpTime(exp_time_each)
    ...
    def resetMultiCondition(self, conditions):
        ...
        if beamline == "BL45XU":
            ...
            trans_percent = obj_trans * 100.0
            attstr = "%6.3f" % trans_percent + "%"
            self.att_thick_multi.SetValue(attstr)
